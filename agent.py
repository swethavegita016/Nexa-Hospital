...
def get_doctors_by_specialty(tool_context: ToolContext, specialty: str = None) -> dict:
    ...
    for name, info in DOCTORS_DB.items():
        if specialty is None or specialty.lower() in info["specialty"].lower() or specialty.lower() in info["department"].lower():
            out.append({
                "name": name,
    ...
    if not out:
        return {"error": f"No doctors for '{specialty}'. Available: Dermatology, Cardiology, Orthopedics, Pediatrics, General Medicine, Gynecology, Neurology"}
...
def get_doctor_slots(tool_context: ToolContext, doctor_name: str, date: str) -> dict:
    ...
    booked = [a["appointment_time"] for a in APPOINTMENTS.values()
              if a["doctor"] == matched and a["appointment_date"] == date and a["status"] != "cancelled"]
    available = [s for s in all_slots if s not in booked]
    return {"doctor": matched, "specialty": info["specialty"], "date": date, "day": day,
            "fee": info["fee"], "available_slots": available, "booked_slots": booked}

def book_appointment(tool_context: ToolContext, patient_name: str, doctor_name: str,
                     appointment_date: str, appointment_time: str, purpose: str = "General consultation") -> dict:
    ...
    info = DOCTORS_DB[matched]
    booked = [a["appointment_time"] for a in APPOINTMENTS.values()
              if a["doctor"] == matched and a["appointment_date"] == appointment_date and a["status"] != "cancelled"]
    if appointment_time in booked:
        return {"error": f"Slot {appointment_time} already taken on {appointment_date}."}
    pid = _new_patient_id()
    APPOINTMENTS[pid] = {
        "patient_id": pid, "patient_name": patient_name, "doctor": matched,
        "specialty": info["specialty"], "department": info["department"],
        "appointment_time": appointment_time, "appointment_date": appointment_date,
        "purpose": purpose, "fee": info["fee"], "status": "confirmed",
        "booked_at": datetime.now().isoformat(),
    }
    db_save_meeting(title=f"{patient_name} – {matched}", purpose=purpose,
    ...
    return {
        ...
        "reminder_note": f"A reminder will be set 30 minutes before your appointment at {appointment_time} on {appointment_date}."
    }
...
def reschedule_appointment(tool_context: ToolContext, patient_id: str, new_date: str, new_time: str, reason: str = None) -> dict:
    ...
    appt.update({"appointment_date": new_date, "appointment_time": new_time, "status": "rescheduled",
                 "reschedule_reason": reason or "Patient request"})
    return {"message": f"Appointment {patient_id} rescheduled.", "patient_id": patient_id,
            "doctor": appt["doctor"], "old_date": old_d, "old_time": old_t,
...
def cancel_appointment(tool_context: ToolContext, patient_id: str, reason: str = None) -> dict:
    ...
    return {"message": f"Appointment {patient_id} cancelled.",
...
def list_appointments(tool_context: ToolContext) -> dict:
    ...
    return {"confirmed": [a for a in APPOINTMENTS.values() if a["status"] == "confirmed"],
...
def set_reminder(tool_context: ToolContext, patient_id: str, patient_name: str,
    ...
    reminder = {
        ...
        "message": f"Reminder: Your appointment with {doctor_name} is in {reminder_minutes} minutes on {appointment_date} at {appointment_time}.",
        "status": "scheduled",
        "created_at": datetime.now().isoformat(),
    }
    REMINDERS.append(reminder)
    db_add_task(
        task=f"🔔 REMINDER [{rid}]: {patient_name} — {doctor_name} on {appointment_date} at {appointment_time} (alert {reminder_minutes} mins before)",
        priority="normal"
    )
    return {
...
try:
    ...
    gcal = MCPToolset(connection_params=StdioConnectionParams(
        server_params=StdioServerParameters(command="python3", args=[os.path.join(BASE, "calendar_mcp_server.py")])))
    print("✅ Local MCP: Calendar connected")
    _appt_tools = [get_doctors_by_specialty, get_doctor_slots, book_appointment,
                   get_appointment, reschedule_appointment, cancel_appointment, list_appointments, gcal]
...
DO NOT show fee, rating, experience or qualifications unless patient asks.
...
AFTER booking is confirmed, automatically set a 30-minute reminder.
Always confirm with:
"🔔 Done! I've set a reminder for 30 minutes before your appointment. You're all set!"

If patient wants a different time (e.g. 1 hour before), adjust accordingly.
